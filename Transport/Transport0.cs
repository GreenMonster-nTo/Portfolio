...
public class Transport0 : Transport
{
    ...
    public Animator transport_anim;                 // 이동수단 애니메이션
    ...
    public Transport_Door[] doors;                  // 문짝 관련
    ...
    private int passenger_kick_amount;              // 승객 쫓아낸 횟수
    ...
    private void TransportInitialize()
    {
        ...
        transport_anim.SetBool("move", false);
        PassengerInitialize();
    ...
    private void PassengerInitialize()
    {
        ...
        for (int i = 0; i < passenger_amount; i++)
        {
            passengers[i].PassengerInitialize(0, i);
        }
    }
    ...
    {
        left_end = new Vector2(19.5f, 3.8f);
        right_end = new Vector2(-19.5f, 3.8f);
    ...
    public void StartTransport()
    {
        TransportStart();
        TouchTimer_Reset();
        transport_anim.SetBool("move", true);
    ...
    private void SetBackground()
    {
        ...
        background_rendrerer.sprite = background_sprites[morning_index];
    }
    ...
    private IEnumerator Move_Background()
    {
        ...
        while (Vector2.Distance(background_bg.transform.localPosition, bg_move) >= 0.1f)
        {
            move_time += Time.deltaTime * 0.85f;
            background_bg.transform.localPosition = new Vector2(Mathf.Lerp(move.x, bg_move.x, move_time / transport_timer), bg_move.y);
            if (Vector2.Distance(background_bg.transform.localPosition, bg_move) < 0.1f)
            {
                transport_anim.SetBool("move", false);
                GameManager.manager.GetSoundManager().SubwayDoor();
            }
            yield return null;
        }
    }
